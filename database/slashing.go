...
func (db *Db) SaveValidatorsSigningInfos(infos []types.ValidatorSigningInfo) error {
	...
	var excludeEmptyInfos []types.ValidatorSigningInfo
	for _, info := range infos {
		if info.ValidatorAddress != "" {
			excludeEmptyInfos = append(excludeEmptyInfos, info)
		}
	}
...
VALUES `
	...
	for i, info := range excludeEmptyInfos {
		ii := i * 7
...
ON CONFLICT (validator_address) DO UPDATE 
	SET start_height = excluded.start_height,
